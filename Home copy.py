...
def ui_run_knowledge_synthesis(config: dict):
    """專為 UI 設計的知識合成函式"""
    # --- 在函式最開始，確保 session_state 中的標記存在 ---
    if 'data_updated' not in st.session_state:
        st.session_state.data_updated = False
        
    synthesis_successful = False # 新增一個標記來追蹤是否有成功合成
    ...
    for i, item in enumerate(new_items):
        ...
        with st.expander(f"處理項目 {i+1}/{total_items}: {content_to_process[:80]}...", expanded=True):
            try:
                ...
                if result:
                    ...
                    synthesis_successful = True # <--- 在成功合成後，設置標記                        
                else:
        ...
        if i < total_items - 1:
            ...
            time.sleep(delay)
    # --- 在函式結尾，根據標記更新 session_state ---
    if synthesis_successful:
        st.session_state.data_updated = True
        st.success("✅ 知識合成流程全部完成！")
    else:
        # 如果整個流程跑完都沒有成功合成任何一項，也要給出提示
        st.success("✅ 知識合成流程結束，沒有新的成功合成。")
